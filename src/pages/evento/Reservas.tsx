...
export default function Reservas() {
    ...
    const precios = useMemo(() => preciosCrud.items.filter(p => p.eventoId === eventoId), [preciosCrud.items, eventoId]);
    const ubicHabilitadas = useMemo(() => ubicCrud.items.filter(u => u.eventoId === eventoId && u.habilitado), [ubicCrud.items, eventoId]);
    const selectores = useMemo(() => {
        const rows = selCrud.items.filter(s => s.eventoId === eventoId && s.habilitado).sort((a,b)=>a.orden-b.orden);
        return {
    ...
    const rows = useMemo(() => reservasCrud.items.filter(r => r.eventoId === eventoId), [reservasCrud.items, eventoId]);
    ...
    function onSubmit(e: React.FormEvent<HTMLFormElement>) {
        ...
        type ReservaInput = Omit<Reserva, keyof BaseEntity>;
        ...
        <section className="space-y-4">
                    ...
                    {rows.map(r => {
                        ...
                        return (
                            <tr key={r.id} className={bloqueado ? 'opacity-60' : ''}>
                                ...
                                <td>{r.receptor ?? ''}</td>
                                <td>{r.totalPedido}</td>
                                <td><input type="checkbox" checked={r.pagado} onChange={() => !bloqueado && reservasCrud.update(r.id, { pagado: !r.pagado })} disabled={bloqueado} /></td>
                                <td><input type="checkbox" checked={r.comprobado} onChange={() => !bloqueado && reservasCrud.update(r.id, { comprobado: !r.comprobado })} disabled={bloqueado} /></td>
                                <td className="flex gap-2">
                                    <button className="btn btn-sm" onClick={() => reservasCrud.update(r.id, { locked: !r.locked })}>{r.locked ? '🔓' : '🔒'}</button>
                                    <button className="btn btn-sm" onClick={() => !bloqueado && reservasCrud.remove(r.id)} disabled={bloqueado}>Borrar</button>
                                </td>
                            </tr>
                        );
                    })}
                    {rows.length === 0 && <tr><td colSpan={11} className="text-center text-gray-500 p-4">Sin reservas</td></tr>}
                    </tbody>
                </table>
            </div>
            <Modal title="Nueva reserva" isOpen={open} onClose={() => setOpen(false)}>
                <form className="grid grid-cols-1 md:grid-cols-2 gap-3" onSubmit={onSubmit}>
                    <FormField label="Cliente"><input name="cliente" className="input" required /></FormField>
                    <FormField label="Parrilladas"><input name="parrilladas" type="number" min={0} defaultValue={0} className="input" /></FormField>
                    <FormField label="Picarones"><input name="picarones" type="number" min={0} defaultValue={0} className="input" /></FormField>
                    <FormField label="Tipo de consumo">
                        <select name="tipoConsumo" className="input">
                            {selectores.tipoConsumo.map(v => <option key={v} value={v}>{v}</option>)}
                        </select>
                    </FormField>
                    <FormField label="Punto de Recogida">
                        <select name="puntoRecogidaId" className="input">
                            <option value="">—</option>
                            {ubicHabilitadas.map(u => <option key={u.id} value={u.id}>{u.nombre}</option>)}
                        </select>
                    </FormField>
                    <FormField label="Método de pago">
                        <select name="metodoPago" className="input">
                            {selectores.metodoPago.map(v => <option key={v} value={v}>{v}</option>)}
                        </select>
                    </FormField>
                    <FormField label="Receptor">
                        <select name="receptor" className="input">
                            <option value="">—</option>
                            {selectores.receptor.map(v => <option key={v} value={v}>{v}</option>)}
                        </select>
                    </FormField>
