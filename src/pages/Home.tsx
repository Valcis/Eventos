...
export default function Home(){
  ...
  function onSubmit(ev: React.FormEvent<HTMLFormElement>){
    ...
    setIsSubmitting(false); setIsOpen(false)
  ...
  return (
    <section className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <button className="btn btn-primary" onClick={()=>setIsOpen(true)}>Crear evento</button>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
        {sorted.map(ev=>(
          <Link key={ev.id} to={`/eventos/${ev.id}`} className="card hover:shadow-lg transition">
            ...
            <div className="text-xs text-gray-500">{ev.ubicacionId ? `Ubicación: ${ev.ubicacionId}` : 'Sin ubicación'}</div>
          </Link>
        ))}
        {sorted.length===0 && <div className="card"><p className="text-gray-600">Aún no hay eventos. Crea el primero.</p></div>}
      </div>
      ...
      <Modal title="Crear evento" isOpen={isOpen} onClose={()=>setIsOpen(false)}>
        <form className="grid grid-cols-1 md:grid-cols-2 gap-3" onSubmit={onSubmit}>
          <FormField label="Nombre"><input className="input" name="nombre" /></FormField>
          <FormField label="Fecha"><input className="input" type="date" name="fecha" /></FormField>
          <FormField label="Ubicación (ID)"><input className="input" name="ubicacionId" /></FormField>
          <FormField label="Presupuesto (€)"><input className="input" name="presupuesto" defaultValue={0} /></FormField>
